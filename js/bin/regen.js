...
var M = {
  ...
  testcasesFile: path.join(__dirname, '../../test/resources/test_capabilities.json'),
  ...
  renew: function (tc) {
    ...
    ['device', 'user', 'browser'].forEach(function (p) {
      if (!tc.capabilities && capabilities) { tc.capabilities = {} }
      if (!tc.capabilities[p] && capabilities[p]) { tc.capabilities[p] = {} }
    })
